...
export function MergePatternVisualizer({ currentStep, className }: MergePatternVisualizerProps) {
  ...
  const mergeHistory: MergeHistoryItem[] = (payload.mergeHistory as MergeHistoryItem[]) || [];
  ...
  return (
    <div className={cn('glass-panel p-4 overflow-hidden', className)}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-foreground">Optimal Merge Pattern</h3>
        <div className="flex gap-4 text-xs">
          <span className="text-muted-foreground">
            Total Cost: <span className="text-success font-mono">{totalCost}</span>
          </span>
          {mergeCost !== undefined && (
            <span className="text-muted-foreground">
              Last Merge: <span className="text-secondary font-mono">+{mergeCost}</span>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="text-xs text-muted-foreground mb-2">Priority Queue (Min-Heap)</div>
        <div className="flex items-end gap-2 h-32 p-2 bg-panel rounded-lg">
          {heap.map((size, idx) => {
            const heightPercent = (size / maxSize) * 100;
            const isSelected = selected.includes(size) && (
              idx === 0 || idx === 1 || 
              (selected[0] === size && heap.indexOf(selected[0]) === idx) ||
              (selected[1] === size && heap.slice(1).indexOf(selected[1]) + 1 === idx)
            );
            ...
            return (
              ...
              >
                ...
                >
                  <span className="text-xs font-mono text-foreground pb-1">{size}</span>
                </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {activeNodes.length === 0 && (
            <div className="text-muted-foreground text-sm">All files merged</div>
          )}
        </div>
      </div>
          ...
          {mergeHistory.length === 0 ? (
            ...
            mergeHistory.map((merge, idx) => (
              ...
              >
                ...
                <span className="font-mono text-foreground">{merge.first}</span>
                <span className="text-muted-foreground">+</span>
                <span className="font-mono text-foreground">{merge.second}</span>
                <span className="text-muted-foreground">=</span>
                <span className="font-mono text-primary">{merge.result}</span>
                <span className="ml-auto text-secondary">cost: +{merge.cost}</span>
              </div>
            ))
          )}
        </div>
